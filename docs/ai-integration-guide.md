...
Skill 是定义 Agent 行为的核心文件，良好的 Skill 设计能有效避免"工具强迫症"（重复调用工具）、"状态迷失"（不知道当前状态）等问题。
...
description: 技能描述          # LLM 可见，用于技能匹配
version: 1.0.0                # 版本号
author: Team Name             # 作者
triggers:                     # 触发条件
  keywords:                   # 关键词触发
...
tools:                        # 工具配置
  allowed:                    # 允许的工具列表
    - tool1
    - tool2
  restricted:                 # 禁止的工具列表（可选）
    - dangerous_tool
  default_permission: allow   # 默认权限（allow/deny）
---
...
防止 Agent 重复调用工具（"工具强迫症"）：

```markdown
### navigate_to_page 使用规则

**触发条件**：用户明确包含动作动词（"打开"、"去"、"跳转"）

**不触发条件**：
- 用户问"这个任务进度多少？" → 用 query_tasks 回答
- 用户说"帮我更新状态" → 用 manage_task 更新

**生命周期**：导航完成后，该工具的"生命周期"结束。
后续对话不再触发导航，除非用户明确要求"刷新"或"切换页面"。
```

**关键**：定义工具何时触发、何时结束，避免重复调用。
...
1. **数据未找到**：告诉用户未找到，建议使用其他关键词
2. **多个结果匹配**：列出匹配项，让用户选择
3. **操作不支持**：明确说明无法执行，并提供替代方案

示例：
用户: "删除任务A"
Agent: "抱歉，我无法直接删除任务。你可以打开任务详情页后手动删除。
需要我帮你打开任务A的详情页吗？"
```

**关键**：优雅地处理错误，而不是让 Agent 困惑。
...
### 示例 2：导航后继续对话（不重复导航）
用户: "打开任务A"
Agent: [调用 navigate_to_page]
Agent: "已打开任务A的详情页。"

用户: "这个任务的进度是多少？"
Agent: [调用 query_tasks]  ← 注意：不再导航
Agent: "任务A的当前进度是 60%。"
...
Agent: [调用 navigate_to_page]
用户: "这个任务的负责人是谁？"
Agent: [调用 navigate_to_page]  ← 错误：重复导航
```

**关键**：正反示例让 Agent 更清楚什么是正确行为。
...
| 问题 | 原因 | 解决方案 |
|------|------|---------|
| 重复调用工具 | 缺少生命周期定义 | 添加工具的触发/结束条件 |
| 选择错误工具 | 缺少优先级指导 | 添加工具选择优先级 |
| 执行不支持的操作 | 缺少能力边界定义 | 明确列出不能做的事 |
| 回复格式混乱 | 缺少格式约束 | 添加回答要求章节 |
| 错误处理不当 | 缺少错误处理指导 | 添加错误处理示例 |
...
### 工具选择优先级
1. **navigate_to_page**：用户说"打开"、"去"
2. **query_tasks**：用户问"有哪些"、"显示"
3. **manage_task**：用户说"创建"、"更新"
